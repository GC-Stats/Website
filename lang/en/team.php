<?php

return [
    'title' => [
        'index' => ':team',
        'history' => ':team - Player History',
        'matches' => ':team - Matches',
        'maps' => ':team - Maps',
    ],

    'players' => 'Current Roster',
    'old_players' => 'Former Members',
    'news' => 'News',
    'seemore' => 'See more',

    // Other pages text
    'matches_history' => ':team – Matches History',
    'players_history' => ':team – Players History',

    'empty' => [
        'matches_history' => 'This team has no matches',
        'players_history' => 'This team has no past players',
        'maps' => 'This team has no maps played yet',
    ],

    'nav' => [
        'aria_label' => 'Team navigation',
        'overview' => 'Overview',
        'matches' => 'Matches',
        'maps' => 'Maps',
        'players_history' => 'Players history',
    ],

    'maps' => [
        'title' => ':team – Maps',
        'record' => 'Record',
        'atk_wr' => 'ATK Win%',
        'def_wr' => 'DEF Win%',
        'pick_rate' => 'Agent pick rate',
        'comps' => 'Comps played',
        'vs' => 'vs',
        'insights' => [
            'title' => 'Insights',
            'most_played' => 'Most played map',
            'least_played' => 'Least played map',
            'best_record' => 'Best record',
            'best_winrate' => 'Best win rate',
            'best_atk' => 'Best ATK win rate',
            'best_def' => 'Best DEF win rate',
            'times' => ':count times',
            'face_to_face' => 'Face to Face',
        ],
    ],

    'roles' => [
        'title' => 'Team roles',
        'back_to_team' => 'Back to :team',
        'member_count' => ':count member|:count members',
        'manage' => 'Manage',
        'delete' => 'Delete role',
        'delete_confirm' => 'Delete the :role role? This removes it from every member who has it.',
        'new_role' => [
            'title' => 'New role',
            'name_label' => 'Role name',
            'submit' => 'Create role',
        ],
        'permissions' => [
            'title' => 'Permissions',
            'save' => 'Save permissions',
            'empty_ceiling' => 'This team has not been granted any permissions yet — ask a site admin to set them.',
        ],
        'members' => [
            'title' => 'Members',
            'empty' => 'No members with this role yet.',
            'remove' => 'Remove',
            'remove_confirm' => 'Remove the :role role from :name?',
            'add' => 'Add a member',
            'search_placeholder' => 'Name or email',
            'search_submit' => 'Search',
            'search_empty' => 'No matching user without this role.',
            'assign' => 'Assign',
        ],
        'errors' => [
            'owner_role_protected' => 'The team_owner role cannot be deleted.',
            'owner_role_admin_only' => 'Assigning a new team_owner is a site admin action (team admin page).',
            'last_owner' => 'Cannot remove the last team_owner — assign another one first.',
        ],
        'status' => [
            'role-created' => 'Role created.',
            'role-deleted' => 'Role deleted.',
            'role-assigned' => 'Role assigned.',
            'role-removed' => 'Role removed.',
            'permissions-updated' => 'Permissions updated.',
        ],
    ],

    'edit' => [
        'title' => 'Edit team',
        'logo' => [
            'title' => 'Logo',
            'submit' => 'Upload',
            'history_title' => 'Logo history',
            'history_from' => 'From',
            'history_until' => 'Until',
            'history_visible' => 'Show on old matches',
            'history_add' => 'Add to history',
            'history_remove_confirm' => 'Permanently remove this logo history entry?',
            'history_empty' => 'No past logos.',
            'theme_label' => 'Theme',
            'theme_universal' => 'All themes',
            'theme_dark' => 'Dark theme only',
            'theme_light' => 'Light theme only',
        ],
        'profile' => [
            'title' => 'Profile',
            'submit' => 'Save changes',
        ],
        'name_history' => [
            'title' => 'Name history',
            'body' => 'Past names this team has been known under, used to show the right name on matches played before a rename.',
            'name_label' => 'Name',
            'from' => 'From',
            'until' => 'Until',
            'visible' => 'Show on old matches',
            'add' => 'Add to history',
            'remove_confirm' => 'Permanently remove this name history entry?',
            'empty' => 'No past names — this team has never been renamed.',
        ],
        'tags' => [
            'title' => 'Fan tags',
            'body' => 'Fans can pick one of these tags on their account settings to show off as "fan of" this team on their public profile (e.g. "G2WIN").',
            'placeholder' => 'e.g. G2WIN',
            'add' => 'Add a tag',
            'remove' => 'Remove',
            'submit' => 'Save tags',
            'empty' => 'No fan tags yet.',
        ],
        'fields' => [
            'name' => 'Team name',
            'short_name' => 'Short name',
            'country_code' => 'Country code',
            'country_code_search' => 'Search for a country…',
            'country_code_none' => 'No country / international',
            'bio' => 'Description',
            'website' => 'Website',
            'vlr_id' => 'VLR.gg ID',
            'vlr_id_info' => 'Not displayed or shared publicly — used internally to simplify our work.',
            'liquipedia_link' => 'Liquipedia link',
            'socials' => 'Social accounts',
        ],
        'status' => [
            'profile-updated' => 'Profile updated.',
            'logo-updated' => 'Logo updated.',
            'roster-member-added' => 'Player added to the roster.',
            'roster-synced' => 'Roster saved.',
            'logo-history-added' => 'Logo added to history.',
            'logo-history-updated' => 'Logo history entry updated.',
            'logo-history-removed' => 'Logo history entry removed.',
            'name-history-added' => 'Name added to history.',
            'name-history-updated' => 'Name history entry updated.',
            'name-history-removed' => 'Name history entry removed.',
            'tags-updated' => 'Fan tags updated.',
            'change-request-submitted' => 'Thanks — your request was submitted for review.',
        ],
    ],

    'change_request' => [
        'trigger' => 'Suggest an edit',
        'title' => 'Suggest an edit — :team',
        'intro' => 'Only fill in the fields you want to change — everything else is left as is. A staff member reviews every request before it goes live.',
        'profile_section' => 'Profile',
        'socials_section' => 'Social accounts',
        'socials_help' => "Enter the username, not a full link — except for Discord, which has no profile link.",
        'socials_placeholder_username' => 'username',
        'socials_placeholder_discord' => 'username or invite link',
        'logo_section' => 'Logo',
        'logo_label' => 'New logo',
        'logo_current' => 'Current logo',
        'logo_help' => 'JPG, PNG or WebP, 10 MB max.',
        'roster_section' => 'Roster',
        'roster_intro' => 'Edit an existing entry\'s role or dates, remove a player, or add a new one — every change here is submitted together with the rest of this form.',
        'roster_add' => 'Add a player',
        'new_player_label' => 'New player',
        'history_section' => 'Roster history',
        'history_intro' => "Correct the role or dates on an existing roster entry — to add a new player, use the \"Roster\" section above.",
        'history_empty' => 'No roster entries.',
        'history_left_at' => 'Left date (leave empty if still on the team)',
        'note_section' => 'Note for the reviewer',
        'note_label' => 'Note for the reviewer (optional)',
        'note_placeholder' => 'Any extra context — a source, why this changed…',
        'submit' => 'Submit request',
        'submitted_status' => 'Thanks — your request was submitted for review.',
        'errors' => [
            'no_changes' => 'Change at least one field before submitting.',
            'joined_at_required' => 'Pick a join date for the proposed roster addition.',
            'logo_too_large' => 'The logo image may not be larger than 10 MB.',
        ],
    ],

    'roster' => [
        'title' => 'Roster',
        'history_title' => 'Roster history',
        'add' => 'Add a player',
        'role' => 'Role',
        'roles' => [
            'player-igl' => 'Player (IGL)',
            'player' => 'Player',
            'sub' => 'Sub',
            'coach' => 'Coach',
            'assistant coach' => 'Assistant Coach',
            'performance coach' => 'Performance Coach',
            'mental coach' => 'Mental Coach',
            'analyst' => 'Analyst',
            'manager' => 'Manager',
            'player-igl-inactive' => 'Player (IGL) (Inactive)',
            'player-inactive' => 'Player (Inactive)',
            'sub-inactive' => 'Sub (Inactive)',
            'coach-inactive' => 'Coach (Inactive)',
            'assistant coach-inactive' => 'Assistant Coach (Inactive)',
            'performance coach-inactive' => 'Performance Coach (Inactive)',
            'mental coach-inactive' => 'Mental Coach (Inactive)',
            'analyst-inactive' => 'Analyst (Inactive)',
            'manager-inactive' => 'Manager (Inactive)',
        ],
        'joined_at' => 'Joined',
        'left_at' => 'Left',
        'save' => 'Save',
        'assign' => 'Add',
        'remove' => 'Remove',
        'remove_confirm' => 'Permanently remove this roster entry for :player?',
        'undo' => 'Undo',
        'current_empty' => 'No players on the current roster.',
        'history_empty' => 'No past roster entries.',
        'since' => 'Since',
        'unknown_date' => 'UNKNOWN',
        'now' => 'Now',
        'inactive_since' => 'Inactive since :date',
    ],
];
