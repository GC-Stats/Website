<?php
return [
    'title' => 'Structure des données',
    'styled_title' => 'Structure <span class="text-gc-yellow">des données</span>',
    'subtitle' => 'Structure et liste des données collectés',
    'opendata' => [
        'title' => 'Portail Open Data',
        'body' => 'Vous souhaitez explorer ou télécharger directement nos données ? Consultez notre portail Open Data.',
        'btn' => 'Visiter le portail Open Data',
    ],
    'titles' => [
        'player_team' => 'Structure des joueuses & équipes',
        'tournament' => 'Structure des tournois',
        'matches' => 'Structure des matches',
        'news' => 'Structure des news',
        'account' => 'Comptes utilisateurs & modération',
        'others' => 'Structure d\'autres données',
    ],
    'descriptions' => [
        'player_team' => "Chaque équipe/joueur doit avoir un nom et un identifiant, les autres informations sont ajoutés par l'équipe/joueuse ou par les administrateurs via les données d'un tournoi ou de Liquipedia.<br>Les joueuses/teams ont un droit à la modification/supression sur les 'Additional Informations'<br><br>Les val_id unique et discord_id unique ne sont pas affichés ou partagés. Ils sont fournis par la joueuse via Discord.",
        'tournament' => 'Chaque tournoi doit avoir un identifiant, un nom, des dates, une région, une category et un status. Ils sont récupérés par notre équipe, ou sur demande lors d\'une demande d\'ajout de tournoi.',
        'matches' => "Nous récoltons beaucoup d'informations sur les matchs afin de fournir des statistiques détaillés. Nous récupérons : <br> - Les informations de base du match (Obligatoire)<br> - les vetos du match <br> - Les maps joués, avec le résultat et les stats <br> - Les statistiques et statistiques avancés (Dans un dataset séparer, référencé par map id)<br> - Le résultat de chaque round (Qui gagne, et comment) <br> - Les stats de chaque joueuse par round (KDA/Economie/Arme/Armure/etc) <br><br>Ces stats ne sont pas toujours récoltés, mais nous essayons de les obtenirs afin d'afficher les statistiques les plus complète possible",
        'news' => 'Chaque news doit avoir un identifiant, un titre, un contenu et un auteur<br><br>Le status est masqué mais permets l\'affichage (Draft = En cours de rédaction, Published = Visible sur le site, Archived = Masqué sur le site mais accessible via l\'API)',
        'account' => "Ces tables n'existent que pour les personnes ayant créé un compte utilisateur (distinct d'un profil de joueuse), nécessaire pour réagir aux news, gérer un profil ou administrer le site.<br><br>Les données de sécurité (mot de passe, 2FA, passkeys) ne sont jamais partagées. Les comptes Discord/Twitter/Twitch liés stockent un jeton d'accès utilisé uniquement pour l'authentification.<br><br>Les données de modération (sanctions, empreintes anti-contournement, signalements, journal d'activité) forment un dossier de sécurité conservé indépendamment du compte, y compris après sa suppression, afin d'empêcher le contournement d'un bannissement.",
        'others' => 'Nous stockons le nombre de visite sur chaque page par heure, nous ne stockons pas par qui, depuis quel region, ou l\'heure exact.<br><br>Ces données sont stockés et privées. Elle ne sont jamais partagées, nous les utilisons afin de visualiser l\'usage du site et d\'adapter notre infrastructure au besoin',
    ],
    'players' => [
        'titles' => [
            'main' => 'Main Informations',
            'additional' => 'Additional Informations',
            'confidential' => 'Confidential informations',
        ],
        'id' => 'Identifiant unique attribué une joueuse (Visible dans l\'url)',
        'handle' => 'Pseudo',
        'first_name' => 'Prénom',
        'last_name' => 'Nom',
        'country_code' => 'Code de pays (FR = France, US = United States, etc)',
        'bio' => 'Biographie',
        'socials' => 'Réseaux Sociaux (Twitter/Instagram/Twitch/Youtube/Tik Tok)',
        'discord_id' => 'Identifiant Discord (Informations privée, utilisé pour valider l\'identité d\'une joueuse lors d\'une modification de son profil)',
        'val_id' => 'Identifiant Riot (Informations privée, identifié et assigné par notre équipe à partir des données de match Riot, utilisé pour lié les statistiques des matchs à une joueuse)',
        'vlr_id' => 'Identifiant VLR',
        'liquipedia_link' => 'Lien de la page Liquipedia d\'une joueuse',
    ],
    'player_team' => [
        'cascade' => 'La suppression d\'un joueur ou d\'une équipe entraîne la suppression automatique des élements liés.',
        'player_id' => 'Identifiant unique d\'une joueuse',
        'team_id' => 'Identifiant unique d\'une team',
        'role' => 'Joueuse/Coach/Manager/etc',
        'joined_at' => 'Date d\'arrivée dans l\'équipe',
        'left_at' => 'Date de départ de l\'équipe',
    ],
    'teams' => [
        'titles' => [
            'main' => 'Main Informations',
            'additional' => 'Additional Informations',
        ],
        'id' => 'Identifiant unique attribué une team (Visible dans l\'url)',
        'name' => 'Nom de l\'équipe',
        'short_name' => 'Nom court (Fnatic = FNC)',
        'country_code' => 'Code de pays (FR = France, US = United States, etc)',
        'bio' => 'Biographie',
        'website' => 'Site internet',
        'socials' => 'Réseaux Sociaux (Twitter/Instagram/Twitch/Youtube/Tik Tok)',
        'vlr_id' => 'Identifiant VLR',
        'liquipedia_link' => 'Lien de la page Liquipedia d\'une équipe',
    ],
    'tournaments' => [
        'titles' => [
            'main' => 'Main Informations',
            'additional' => 'Additional Informations',
        ],
        'id' => 'Identifiant unique du tournoi',
        'name' => 'Nom de la compétition',
        'region' => 'Région du monde (EMEA, NA, etc)',
        'category' => 'Catégorie du tournoi (ex: Challengers, Cash Cups)',
        'prize_pool' => 'Récompense totale (Cashprize)',
        'location' => 'Lieu physique',
        'start_date' => 'Date de début',
        'end_date' => 'Date de fin',
        'status' => 'Statut actuel (À venir, En cours, Terminé)',
        'description' => 'Présentation de l\'événement',
        'liquipedia_link' => 'Lien de la page Liquipedia d\'un tournoi',
    ],
    'tournament_phases' => [
        'titles' => [
            'structure' => 'Structure technique',
        ],
        'tournament_id' => 'Référence au tournoi concerné',
        'name' => 'Nom de l\'étape (ex: Playoffs, Groupe A)',
        'format' => 'Mode de jeu (ex: Élimination directe, Round Robin)',
        'parent_id' => 'Lien vers une étape parente (pour les sous-groupes)',
        'order' => 'Ordre d\'affichage de l\'étape',
    ],
    'tournament_teams' => [
        'link' => 'Table de liaison : Connecte les équipes aux tournois auxquels elles participent.',
        'team_list' => 'Liste des équipes inscrites à l\'événement',
    ],
    'matches' => [
        'titles' => [
            'structure' => 'Structure technique',
        ],
        'id' => 'Identifiant unique du match',
        'tournament_id' => 'Référence du tournoi',
        'phase_id' => 'Référence de l\'étape (ex: Playoffs)',
        'round_number' => 'Numéro du tour (ex: Tour 1)',
        'round_name' => 'Nom du tour (ex: Grand Final)',
        'match_order' => 'Ordre de passage',
        'team_a_id' => 'Identifiant unique Équipe A',
        'team_b_id' => 'Identifiant unique Équipe B',
        'scheduled_at' => 'Date et heure de début',
        'status' => 'Statut (upcoming, live, finished)',
        'team_a_score' => 'Score final Équipe A',
        'team_b_score' => 'Score final Équipe B',
        'best_of' => 'Nombre de maps maximum (BO1, BO3, BO5)',
        'patch' => 'Version du jeu (ex: 8.04)',
    ],
    'match_vetos' => [
        'id' => 'Identifiant unique du veto',
        'match_id' => 'Match concerné',
        'team_id' => 'Équipe effectuant l\'action',
        'map_name' => 'Nom de la carte (ex: Ascent)',
        'type' => 'Action (Pick, Ban ou Decider)',
        'order' => 'Ordre dans la séquence de veto',
    ],
    'game_maps' => [
        'id' => 'Identifiant unique de la map jouée',
        'api_match_id' => 'Identifiant de la carte, donné par l\'API Riot',
        'match_id' => 'Match concerné',
        'map_name' => 'Nom de la carte',
        'team_a_score' => 'Score équipe A sur cette map',
        'team_b_score' => 'Score équipe B sur cette map',
        'order' => 'Position de la map dans le match',
        'is_completed' => 'La carte est-elle terminée ?',
    ],
    'game_player_stats' => [
        'id' => 'Identifiant unique de la stat',
        'match_id' => 'Match concerné',
        'game_map_id' => 'Carte concernée',
        'player_id' => 'Joueuse concernée',
        'team_id' => 'Équipe au moment du match',
        'agent_name' => 'Agent joué (ex: Jett)',
        'kills' => 'Éliminations totales',
        'deaths' => 'Morts totales',
        'assists' => 'Assistances totales',
        'acs' => 'Score de combat moyen',
        'adr' => 'Dégâts moyens par round',
        'kast_percentage' => '% de rounds avec Kill/Assist/Survival/Trade',
        'first_kills' => 'Premiers sangs (FK)',
        'first_deaths' => 'Premières morts (FD)',
        'headshot_percentage' => '% de tirs à la tête',
    ],
    'game_map_rounds' => [
        'id' => 'Identifiant unique du round',
        'game_map_id' => 'Carte concernée',
        'round_number' => 'Numéro du round (1-24+)',
        'winning_team' => 'Équipe victorieuse',
        'win_type' => 'Type de victoire (Defuse, Detonation, Wipe)',
        'atk_team' => 'Équipe en attaque ce round',
        'def_team' => 'Équipe en défense ce round',
        'plant_site' => 'Site de plant du spike (A/B/C)',
        'plant_x' => 'Position X du plant sur la mini-carte',
        'plant_y' => 'Position Y du plant sur la mini-carte',
    ],
    'game_map_round_kills' => [
        'game_map_round_id' => 'Round concerné',
        'killer_player_id' => 'Joueuse ayant réalisé l\'élimination',
        'victim_player_id' => 'Joueuse éliminée',
        'assistant_player_ids' => 'Joueuses ayant assisté l\'élimination',
        'time_ms' => 'Instant de l\'élimination (ms depuis le début du round)',
        'weapon' => 'Arme utilisée',
        'damage_type' => 'Type de dégâts (arme, capacité, chute, etc.)',
        'is_secondary_fire' => 'Élimination au tir secondaire (ex: visée d\'une arme)',
    ],
    'game_map_round_damages' => [
        'game_map_round_id' => 'Round concerné',
        'attacker_player_id' => 'Joueuse ayant infligé les dégâts',
        'receiver_player_id' => 'Joueuse ayant reçu les dégâts',
        'damage' => 'Dégâts infligés',
        'headshots' => 'Nombre de tirs à la tête',
        'bodyshots' => 'Nombre de tirs au corps',
        'legshots' => 'Nombre de tirs aux jambes',
    ],
    'game_map_round_alive_states' => [
        'game_map_round_id' => 'Round concerné',
        'sequence' => 'Ordre de l\'évènement dans le round',
        'time_ms' => 'Instant de l\'évènement (ms depuis le début du round)',
        'atk_alive' => 'Joueuses en vie côté attaque',
        'def_alive' => 'Joueuses en vie côté défense',
        'winner_side' => 'Camp actuellement en avantage numérique',
    ],
    'game_map_round_player_positions' => [
        'game_map_round_id' => 'Round concerné',
        'event_type' => 'Type d\'évènement (élimination, plant, désamorçage)',
        'game_map_round_kill_id' => 'Élimination associée, le cas échéant',
        'player_id' => 'Joueuse concernée',
        'role' => 'Rôle dans l\'évènement (tueuse, victime, témoin, planteuse, désamorceuse)',
        'x' => 'Position X sur la mini-carte',
        'y' => 'Position Y sur la mini-carte',
        'view_radians' => 'Angle de vue au moment de l\'évènement',
        'time_ms' => 'Instant de l\'évènement (ms depuis le début du round)',
    ],
    'game_map_round_player_stats' => [
        'id' => 'Identifiant unique de la stat round',
        'game_map_round_id' => 'Round concerné',
        'player_id' => 'Joueuse concernée',
        'team_id' => 'Équipe de la joueuse au moment du round',
        'kills' => 'Kills durant ce round',
        'assists' => 'Assists durant ce round',
        'score' => 'Score obtenu ce round',
        'economy_spent' => 'Crédits dépensés',
        'economy_remaining' => 'Crédits restants',
        'weapon_id' => 'Arme principale utilisée',
        'armor' => 'Type d\'armure (Light/Heavy)',
    ],
    'news' => [
        'titles' => [
            'main' => 'Main Informations',
            'additional' => 'Additional Informations',
        ],
        'id' => 'Identifiant unique de la news (Visible dans l\'url)',
        'author' => 'Nom de l\'auteur/autrice',
        'title' => 'Titre de la news',
        'content' => 'Contenu de la news',
        'status' => 'Status de la news (draft/published/archived)',
        'is_featured' => 'Si la news est affichés en première page',
        'published_at' => 'Date de publication',
        'relation' => 'Lien entre une news et un tournoi/team/joueuse, il permets de l\'afficher sur le profil.',
    ],
    'others' => [
        'titles' => [
            'main' => 'Main Informations',
        ],
        'uri' => 'URL de la page',
        'viewed_at' => 'Heure de visionnage',
        'count' => 'Nombre de visionnage',
    ],
    'game_player_advanced_stats' => [
        'id' => 'Identifiant unique des stats avancés',
        'game_map_id' => 'Carte concerné',
        'player_id' => 'Joueuse concernée',
        'agent_name' => 'Agent joué',
        'clutch_1v1_won' => 'Clutch gagnés, 1v1',
        'clutch_1v1_total' => 'Clutch joués, 1v1',
        'clutch_1v2_won' => 'Clutch gagnés, 1v2',
        'clutch_1v2_total' => 'Clutch joués, 1v2',
        'clutch_1v3_won' => 'Clutch gagnés, 1v3',
        'clutch_1v3_total' => 'Clutch joués, 1v3',
        'clutch_1v4_won' => 'Clutch gagnés, 1v4',
        'clutch_1v4_total' => 'Clutch joués, 1v4',
        'clutch_1v5_won' => 'Clutch gagnés, 1v5',
        'clutch_1v5_total' => 'Clutch joués, 1v5',
        'multikill_2k' => 'Manches avec 2 éliminations',
        'multikill_3k' => 'Manches avec 3 éliminations',
        'multikill_4k' => 'Manches avec 4 éliminations',
        'multikill_5k' => 'Manches avec 5 éliminations',
        'trade_kills' => 'Éliminations qui ont venger un coéquipier',
        'traded_deaths' => 'Morts vengés par un coéquipier',
        'plants' => 'Plante du spike',
        'defuses' => 'Defuse du spike',
        'pistol_won' => 'Manche au pistolet gagnés',
        'pistol_played' => 'Manche au pistolet joués',
        'eco_won' => 'Round d\'économie gagnés',
        'eco_played' => 'Round d\'économie joués',
        'force_won' => 'Manche forcés gagnés',
        'force_played' => 'Manche forcés joués',
        'full_buy_won' => 'Manche avec un achat complet gagnés',
        'full_buy_played' => 'Manche avec un achat complet joués',
        'post_plant_won' => 'Situation post-plant gagnés',
        'post_plant_played' => 'Situation post-plant joués',
        'atk_rounds' => 'Round joués en attaque',
        'atk_rounds_won' => 'Round gagnés en attaque',
        'atk_kills' => 'Éliminations en attaque',
        'atk_kast_percentage' => 'KAST% en attaque',
        'def_rounds' => 'Round joués en défense',
        'def_rounds_won' => 'Round gagnés en défense',
        'def_kills' => 'Éliminations en défense',
        'def_kast_percentage' => 'KAST% en défense',
    ],
    'users' => [
        'titles' => [
            'main' => 'Main Informations',
            'security' => 'Sécurité (privé)',
        ],
        'id' => 'Identifiant unique du compte',
        'username' => 'Pseudo affiché publiquement',
        'email' => 'Adresse email (privée, jamais affichée)',
        'team_tag' => 'Équipe affichée comme "supportée" sur le profil public, avec un tag de cette équipe',
        'preferences' => 'Préférences d\'interface (privées)',
        'password' => 'Mot de passe, haché (jamais stocké ou affiché en clair)',
        'two_factor_secret' => 'Secret de double authentification (2FA), si activée',
        'two_factor_recovery_codes' => 'Codes de récupération 2FA, si activée',
        'discord_synced_at' => 'Date de dernière synchronisation avec Discord',
    ],
    'social_accounts' => [
        'user_id' => 'Compte utilisateur concerné',
        'provider' => 'Plateforme liée (Discord, Twitter/X ou Twitch)',
        'provider_id' => 'Identifiant du compte sur cette plateforme',
        'nickname' => 'Pseudo sur cette plateforme',
        'avatar' => 'Avatar récupéré depuis cette plateforme',
        'token' => 'Jeton d\'accès OAuth (privé, jamais partagé, utilisé uniquement pour l\'authentification)',
        'refresh_token' => 'Jeton de rafraîchissement OAuth (privé)',
        'token_expires_at' => 'Date d\'expiration du jeton d\'accès',
    ],
    'passkeys' => [
        'name' => 'Nom donné à la clé d\'accès (passkey)',
        'credential_id' => 'Identifiant technique de la clé (privé)',
        'last_used_at' => 'Dernière utilisation de cette clé',
    ],
    'sanctions' => [
        'type' => 'Type de sanction (avertissement, bannissement, etc.)',
        'reason' => 'Motif de la sanction',
        'starts_at' => 'Date de début d\'effet',
        'ends_at' => 'Date de fin (si temporaire)',
        'revoked_at' => 'Date de levée anticipée, le cas échéant',
        'issued_by' => 'Modératrice ayant prononcé la sanction',
        'revoked_by' => 'Modératrice ayant levé la sanction, le cas échéant',
    ],
    'sanction_identities' => [
        'type' => 'Type d\'empreinte technique associée (ex: email)',
        'value' => 'Valeur de l\'empreinte (privée, conservée même après suppression du compte afin d\'empêcher le contournement d\'un bannissement)',
    ],
    'user_reports' => [
        'reporter_id' => 'Compte à l\'origine du signalement',
        'reported_user_id' => 'Compte signalé',
        'category' => 'Catégorie du signalement (fraude, contournement de bannissement, harcèlement, faux compte, autre)',
        'reason' => 'Détail fourni par la personne signalante',
        'status' => 'Statut du traitement (en attente, résolu, rejeté)',
        'reviewed_by' => 'Modératrice ayant traité le signalement',
        'resolution_note' => 'Note de résolution laissée par la modération',
    ],
    'activity_log' => [
        'log_name' => 'Catégorie de l\'action journalisée',
        'description' => 'Description de l\'action effectuée',
        'event' => 'Type d\'évènement (création, modification, suppression)',
        'attribute_changes' => 'Détail des champs modifiés (avant/après)',
        'causer' => 'Compte ayant effectué l\'action',
        'subject' => 'Élément concerné par l\'action (joueuse, équipe, etc.)',
    ],
    'reactions' => [
        'emote_id' => 'Emote utilisée',
        'user_id' => 'Compte ayant réagi',
        'reactable' => 'Contenu concerné par la réaction (ex: une news)',
    ],
    'emotes' => [
        'name' => 'Nom de l\'emote',
        'image_path' => 'Chemin de l\'image',
        'source' => 'Origine de l\'emote (ex: twemoji, personnalisée)',
        'is_active' => 'L\'emote est-elle disponible à l\'utilisation ?',
    ],
];
