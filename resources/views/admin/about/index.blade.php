        ...
        <section>
            ...
            <div class="space-y-4">
                @foreach ($sections as $key => $section)
                    ...
                    @endphp
                    <div class="bg-bg-card border border-border-subtle rounded-sm p-4">
                            ...
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <input type="number" id="order-{{ $key }}" name="order" value="{{ $isResubmit ? old('order') : $section->order }}"
                                           class="w-16 bg-[#050505] border border-border-subtle rounded-sm px-2 py-1.5 text-xs text-white focus:outline-none focus:border-gc-yellow transition">
                                </div>
                            </div>
                            ...
                            @foreach ($locales as $locale)
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="content[{{ $locale }}]" rows="3"
                                                  class="w-full bg-[#050505] border border-border-subtle rounded-sm px-4 py-3 text-sm text-white focus:outline-none focus:border-gc-yellow transition">{{ $isResubmit ? old('content.'.$locale) : ($section->content[$locale] ?? '') }}</textarea>
                                        @if ($isResubmit)
                ...
                @forelse ($team as $member)
                    <div class="bg-bg-card border border-border-subtle rounded-sm p-4">
                        <div class="flex items-center gap-3 mb-3">
                            @if ($member->photo_url)
                                <img src="{{ $member->photo_url }}" alt="" class="w-12 h-12 rounded-full object-cover border border-border-subtle">
                            @else
                                <div class="w-12 h-12 rounded-full bg-white/5 border border-border-subtle flex items-center justify-center text-gray-600">
                            ...
                            @unless ($member->is_active)
                                <span class="ml-auto shrink-0 px-2 py-1 text-[9px] font-bold uppercase tracking-widest rounded-sm bg-gray-500/10 text-gray-400 border border-gray-500/30">
                        ...
                        @can('about.manage')
                            <div class="flex gap-2">
                                <x-modal :title="$member->name" max-width="max-w-lg">
                                    <x-slot:trigger>
                                        <button type="button"
                                                class="flex-1 font-bold uppercase text-[10px] tracking-widest px-3 py-1.5 rounded-sm transition active:scale-95 bg-white/5 border border-border-subtle text-white hover:bg-white/10">
                                            {{ __('admin.about.edit') }}
                                        </button>
                                    </x-slot:trigger>
                                    @include('admin.about._member-form', ['member' => $member])

                                    <form method="POST" action="{{ route('admin.about.team.photo', $member) }}" enctype="multipart/form-data" class="mt-4 pt-4 border-t border-t-border-subtle space-y-2">
                                        @csrf
                                        <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500">
                                            {{ __('admin.about.member_photo') }}
                                        </label>
                                        <input type="file" name="image" accept="image/*" required
                                               class="w-full text-xs text-gray-400 file:mr-3 file:py-2 file:px-3 file:rounded-sm file:border-0 file:text-[10px] file:font-bold file:uppercase file:tracking-widest file:bg-white/5 file:text-white">
                                        <button type="submit"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse ($projects as $project)
                    <div class="bg-bg-card border border-border-subtle rounded-sm p-4">
                        <div class="flex items-center gap-3 mb-3">
                            @if ($project->logo_url)
                                <img src="{{ $project->logo_url }}" alt="" class="w-12 h-12 rounded-sm object-cover border border-border-subtle">
                            @else
                                <div class="w-12 h-12 rounded-sm bg-white/5 border border-border-subtle flex items-center justify-center text-gray-600">
                        ...
                        @can('about.manage')
                            <div class="flex gap-2">
                                <x-modal :title="$project->name" max-width="max-w-lg">
                                    ...
                                    <form method="POST" action="{{ route('admin.about.projects.logo', $project) }}" enctype="multipart/form-data" class="mt-4 pt-4 border-t border-t-border-subtle space-y-2">
                                        @csrf
                                        <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500">
