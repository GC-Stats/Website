...
@section('content')
    @if (session('reveal_url'))
        <div x-data="{ copied: false }" class="mb-6 bg-gc-yellow/10 border border-gc-yellow/40 rounded-sm px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
            <div>
    ...
    <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <form method="GET" class="flex-1 min-w-[200px] max-w-sm">
            <input type="text" name="q" value="{{ $search }}" placeholder="{{ __('admin.api_keys.search_placeholder') }}"
                   class="w-full bg-[#050505] border border-border-subtle rounded-sm px-4 py-2.5 text-sm text-white focus:outline-none focus:border-gc-yellow transition">
        </form>
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="border-b border-border-subtle text-[10px] font-black uppercase tracking-widest text-gray-500">
                    <th class="px-4 py-3">{{ __('admin.api_keys.client_name') }}</th>
                    <th class="px-4 py-3">{{ __('admin.api_keys.rate_limit') }}</th>
                    <th class="px-4 py-3">{{ __('admin.api_keys.status') }}</th>
                        ...
                        <td class="px-4 py-3">
                            @can('api-keys.manage')
                                <form method="POST" action="{{ route('admin.api-keys.toggle', $key) }}">
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                            class="px-2 py-1 text-[10px] font-bold uppercase tracking-widest rounded-sm transition {{ $key->is_active ? 'bg-green-500/10 text-green-400 border border-green-500/30 hover:bg-green-500/20' : 'bg-gray-500/10 text-gray-400 border border-gray-500/30 hover:bg-gray-500/20' }}">
                                        {{ $key->is_active ? __('admin.api_keys.active') : __('admin.api_keys.inactive') }}
                                    </button>
                                </form>
                            @else
                                <span class="px-2 py-1 text-[10px] font-bold uppercase tracking-widest rounded-sm {{ $key->is_active ? 'bg-green-500/10 text-green-400 border border-green-500/30' : 'bg-gray-500/10 text-gray-400 border border-gray-500/30' }}">
                                    {{ $key->is_active ? __('admin.api_keys.active') : __('admin.api_keys.inactive') }}
                                </span>
                            @endcan
                        </td>
