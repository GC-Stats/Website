                ...
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="border-b border-white/10 text-[10px] font-black uppercase tracking-widest text-gray-500">
                            @foreach ([['phase', 'admin.matches.phase'], ['round_name', 'admin.matches.round_name'], ['team', 'admin.matches.teams'], ['status', 'admin.matches.status_column'], ['date', 'admin.matches.scheduled_at']] as [$col, $label])
                    ...
                    <tbody>
                        @forelse ($matches as $match)
                            <tr class="border-b border-white/10 last:border-0">
                                <td class="px-4 py-3 text-xs font-bold uppercase text-gray-400">{{ $match->tournamentPhase->name ?? '—' }}</td>
                                <td class="px-4 py-3 text-xs uppercase text-gray-500">{{ $match->round_name ?: '—' }}</td>
                                <td class="px-4 py-3 text-white font-semibold">
                                    {{ \App\Support\MatchDisplay::teamShortName($match->teamA, $match->status) }} vs {{ \App\Support\MatchDisplay::teamShortName($match->teamB, $match->status) }}
                                    @if (! is_null($match->team_a_score))
                                        ({{ $match->team_a_score }} - {{ $match->team_b_score }})
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <span class="whitespace-nowrap text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg
                                          {{ $match->status === 'finished' ? 'bg-white/5 text-gray-400' : ($match->status === 'live' ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400') }}">
