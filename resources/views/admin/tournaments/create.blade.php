{{--
    GC-Stats — Admin: create tournament
...
@section('title', __('admin.tournaments.create.title'))
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-black uppercase tracking-tighter text-white">{{ __('admin.tournaments.create.title') }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.tournaments.store') }}">
        @csrf

        @include('admin.tournaments._form', ['tournament' => null, 'regions' => $regions, 'categories' => $categories])
        ...
        <button type="submit"
            ...
            {{ __('admin.tournaments.create.submit') }}
        </button>
    </form>
@endsection
