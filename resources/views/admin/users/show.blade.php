...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        @can('reports.view')
            <div class="bg-bg-card border border-white/10 rounded-xl backdrop-blur-sm p-4 lg:col-span-2">
                <p class="text-[10px] font-black uppercase tracking-widest text-gc-yellow mb-3">{{ __('admin.users.reports_title') }}</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('admin.users.reports_received_title') }}</p>
                        @forelse ($reportsReceived as $report)
                            <a href="{{ route('admin.reports.show', $report) }}"
                               class="flex items-center justify-between gap-3 py-2 border-b border-b-white/10 last:border-b-0 text-sm hover:bg-white/5 -mx-1 px-1 rounded transition">
                                <div class="min-w-0">
                                    <span class="text-white">{{ __('admin.reports.category.'.$report->category) }}</span>
                                    <span class="text-gray-500 ml-2 truncate">{{ $report->reason }}</span>
                                </div>
                                <span class="shrink-0 text-xs text-gray-500">{{ __('admin.reports.status.'.$report->status) }}</span>
                            </a>
                        @empty
                            <p class="text-xs text-gray-600">{{ __('admin.users.no_reports_received') }}</p>
                        @endforelse
                    </div>

                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('admin.users.reports_submitted_title') }}</p>
                        @forelse ($reportsSubmitted as $report)
                            <a href="{{ route('admin.reports.show', $report) }}"
                               class="flex items-center justify-between gap-3 py-2 border-b border-b-white/10 last:border-b-0 text-sm hover:bg-white/5 -mx-1 px-1 rounded transition">
                                <div class="min-w-0">
                                    <span class="text-white truncate">{{ $report->reportedUser?->name ?? '—' }}</span>
                                    <span class="text-gray-500 ml-2 truncate">{{ __('admin.reports.category.'.$report->category) }}</span>
                                </div>
                                <span class="shrink-0 text-xs text-gray-500">{{ __('admin.reports.status.'.$report->status) }}</span>
                            </a>
                        @empty
                            <p class="text-xs text-gray-600">{{ __('admin.users.no_reports_submitted') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
