    ...
    <div class="grid grid-cols-12 gap-6">
        <section class="col-span-12 lg:col-span-5 lg:col-start-4 space-y-6">
            ...
            <div class="border-b border-border-subtle pb-6 text-center">
                ...
                <p class="text-sm text-gray-400 mt-2">{{ __('auth.register.subtitle') }}</p>
            </div>
            ...
            <div class="bg-bg-card border border-border-subtle rounded-sm p-6 shadow-xl space-y-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-widest text-gray-500">{{ __('auth.register.or_divider') }}</span>
                    <div class="h-[1px] flex-1 bg-white/10"></div>
                </div>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="username" class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">
                            {{ __('auth.register.username_label') }}
                        </label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">
                            {{ __('auth.register.password_confirmation_label') }}
                        </label>
                ...
                <p class="text-xs text-gray-500 text-center">
                    {{ __('auth.register.has_account_prompt') }}
                    <a href="{{ route('login') }}" class="text-gc-yellow font-semibold hover:underline">{{ __('auth.register.login_link') }}</a>
                </p>
            </div>
        </section>
    </div>
