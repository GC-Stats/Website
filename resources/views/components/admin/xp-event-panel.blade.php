{{--
    GC-Stats — XP panel (event-scoped)

    Bulk roster-style editor for staff_assignments scoped to one Tournament
    or Match — replaces the old single-add staff-assignments-panel. Same
    pre-mounted-slots pattern as x-admin.roster-panel (see its docblock for
    why: Livewire components can't be dynamically instantiated inside a
    plain Alpine x-for).
        ...
        <div class="space-y-3" x-data="{
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @forelse ($entries as $entry)
                    @include('components.partials.xp-event-entry-card', ['entry' => $entry, 'index' => $entryIndex, 'teamRoles' => $teamRoles, 'orgRoles' => $orgRoles, 'removeConfirmBody' => $removeConfirmBody])
                ...
                @for ($i = 0; $i < $maxNewSlots; $i++)
                    <template x-if="activeSlots.includes({{ $i }})">
                        @include('components.partials.xp-event-new-entry-card', ['index' => $newSlotBase + $i, 'slot' => $i, 'teamRoles' => $teamRoles, 'orgRoles' => $orgRoles])
                    </template>
                @endfor
            </div>
