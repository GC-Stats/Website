{{--
    GC-Stats — XP panel (organization-dashboard-scoped)

    Bulk roster-style editor for one tournament/match's XP entries, scoped
    to a single organization from its own dashboard — every entry
    represents that org (organization_id forced server-side), so unlike
    xp-event-panel there's no team/org toggle per row, just who it's for
    (a staff member, or the org itself). Same pre-mounted-slots pattern.
        ...
        <div class="space-y-3" x-data="{
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @forelse ($entries as $entry)
                    @include('components.partials.xp-org-entry-card', ['entry' => $entry, 'index' => $entryIndex, 'orgRoles' => $orgRoles, 'removeConfirmBody' => $removeConfirmBody])
                ...
                @for ($i = 0; $i < $maxNewSlots; $i++)
                    <template x-if="activeSlots.includes({{ $i }})">
                        @include('components.partials.xp-org-new-entry-card', ['index' => $newSlotBase + $i, 'slot' => $i, 'orgRoles' => $orgRoles])
                    </template>
                @endfor
            </div>
