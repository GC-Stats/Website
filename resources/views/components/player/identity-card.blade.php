{{--
    GC-Stats — Player identity card

    The photo/name/country/bio/socials block — extracted from
    public/player/header.blade.php so the exact same visual can be reused
    standalone (no page nav/change-request chrome), e.g. the forum embed's
    "header" variant (see resources/views/components/forum/embed-card.blade.php).
    $player is the same array shape every player page passes ($player->toArray()).
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full">
    <div class="flex items-center gap-4 md:gap-5 w-full">
        <div class="relative flex-shrink-0">
            @if($player['profile_photo'])
                <img src="{{ $player['profile_photo'] }}" alt="{{ $player['handle'] }}"
                     class="w-16 h-16 md:w-32 md:h-32 object-contain border border-white/10 rounded-lg bg-black/40">
            @else
                <div class="w-16 h-16 md:w-32 md:h-32 flex items-center justify-center border border-white/10 rounded-lg bg-[var(--brand-yellow)]/10">
                    <span class="text-2xl md:text-5xl font-black text-[var(--brand-yellow)]">
        ...
        <div class="flex flex-col justify-center min-w-0 flex-grow">
            ...
            <div class="flex items-center gap-3">
                <span class="fi fi-{{ blank($player['country_code']) || $player['country_code'] === 'inter' ? 'un' : strtolower($player['country_code']) }} shadow-sm flex-shrink-0"
    ...
    <div class="flex flex-wrap md:flex-col gap-2 w-full md:w-auto md:min-w-[180px] md:pl-6">
        @php
            ...
            $socials = is_string($player['socials'] ?? null) ? json_decode($player['socials'], true) : ($player['socials'] ?? []);
        @endphp

        @if(!empty($socials) || ($player['liquipedia_link'] ?? null))
            <div class="flex flex-col gap-1 min-w-[150px] justify-center border-t md:border-t-0 md:border-l border-border-subtle pt-4 md:pt-0 md:pl-6">
                @if($player['liquipedia_link'] ?? null)
                    <a href="{{ $player['liquipedia_link'] }}" target="_blank" rel="noopener noreferrer"
                       aria-label="Liquipedia: {{ $player['handle'] }}"
