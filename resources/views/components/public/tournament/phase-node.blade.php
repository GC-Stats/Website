...
@if(!empty($children))
    @if($isLeafBracketGroup)
        <x-public.tournament.pan-zoom-bracket>
            <div class="flex flex-col gap-20">
                @foreach($children as $child)
                    <div class="flex flex-col items-start">
                        ...
                        <x-public.tournament.bracket-grid :matches="$child['matches'] ?? []" />
                    </div>
                @endforeach
            </div>
        </x-public-tournament.pan-zoom-bracket>
        ...
        @endphp
        <x-public.tournament.leaderboard :phase="$combinedPhase" :teams="$teams" />
    @else
        <div class="flex flex-col gap-12">
            @foreach($children as $child)
                <div class="flex flex-col">
                    ...
                    <x-public.tournament.phase-node :node="$child" :teams="$teams" />
                </div>
            @endforeach
        </div>
    @endif
@elseif(in_array($node['format'] ?? '', ['swiss', 'swiss_buchholz'], true))
    <x-public.tournament.swiss-standings
        :matches="$node['matches'] ?? []"
        :phase="$node"
        :teams="$teams"
        :show-buchholz="($node['format'] ?? '') === 'swiss_buchholz'"
    />
    <x-public.tournament.leaderboard :phase="$node" :teams="$teams" />
@elseif(($node['format'] ?? '') === 'round_robin')
    <x-public.tournament.round-robin
        :matches="$node['matches'] ?? []"
        :phase="$node"
        :teams="$teams"
    />
        ...
        @if($showHeading)
            <div class="flex flex-col gap-20">
                <div class="flex flex-col items-start">
                    ...
                    <x-public.tournament.bracket-grid :matches="$node['matches'] ?? []" />
                </div>
            </div>
        @else
            <x-public.tournament.bracket-grid :matches="$node['matches'] ?? []" />
