...
<div
    x-data="{
        ...
        select(v) {
            this.value = v;
            this.open = false;
            // Mirrors a native select's change event so existing @change handlers
            // on the parent element keep working untouched.
            this.$dispatch('change');
            @if($navigate)
                window.location = v;
            @elseif($autosubmit)
                this.$nextTick(() => this.$refs.hidden?.closest('form')?.submit());
            @endif
        },
    }"
    x-modelable="value"
...
>
    ...
    <div x-show="open" x-cloak
         class="absolute z-20 mt-1 w-full min-w-max bg-bg-card border border-white/10 rounded-lg shadow-xl max-h-64 overflow-y-auto">
