{{--
    GC-Stats — Create your author profile, self-service (shared content)

    Shared between admin/news/authors/create-self.blade.php (extends
    admin.layout) and organization/dashboard/news/author/create-self.blade.php
    (extends organization.layout). Expects $routePrefix, $organization
    (nullable), and optionally $backUrl/$backLabel.
...
<div class="max-w-xl mx-auto">
    ...
    <form method="POST" action="{{ route($routePrefix.'store', $organization ?? []) }}" class="bg-bg-card border border-white/10 rounded-xl backdrop-blur-sm p-6 shadow-xl space-y-4">
        @csrf
        <div>
