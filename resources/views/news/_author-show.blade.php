{{--
    GC-Stats — Author profile (shared content, 100% editable by its owner)

    Shared between admin/news/authors/show.blade.php (site-admin view,
    extends admin.layout) and organization/dashboard/news/author/show.blade.php
    (extends organization.layout) — same Admin\NewsAuthorController::show()
    action renders either wrapper depending on which route matched. Expects
    $author, $routePrefix, $organization (nullable — bound only in dashboard
    mode), and optionally $backUrl/$backLabel.
...
<div class="space-y-6">
    <div class="bg-bg-card border border-white/10 rounded-xl backdrop-blur-sm p-6 shadow-xl space-y-4">
        ...
        <x-admin.logo-upload-form
            ...
            :action-url="route($routePrefix.'logo.update', $routeArgs($author))"
    ...
    <div class="bg-bg-card border border-white/10 rounded-xl backdrop-blur-sm p-6 shadow-xl space-y-6">
        <form method="POST" action="{{ route($routePrefix.'update', $routeArgs($author)) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            @if (auth()->user()->can('news.authors.edit') && $author->user_id !== auth()->id())
                <div>
