{{--
    GC-Stats — News author profile page

    Header inspired by the player profile (photo/avatar, handle, bio, socials).
    Lists all published articles by this author with pagination.
...
@php
    ...
    $socials = is_string($author['socials'] ?? null) ? json_decode($author['socials'], true) : ($author['socials'] ?? []);
@endphp

{{-- ── Author header (player pattern) ────────────────────────────────── --}}
        ...
        <div class="p-4 md:p-6 flex flex-col gap-6">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full">
                ...
                <div class="flex items-center gap-4 md:gap-5 w-full">
                    @php $authorLogo = $author['logo']; @endphp
                    <div class="relative flex-shrink-0">
                        @if($authorLogo)
                            <img src="{{ $authorLogo }}" alt="{{ $author['name'] }}"
                                 class="w-16 h-16 md:w-32 md:h-32 object-cover border border-white/10 rounded-lg bg-black/40">
                        @else
                    ...
                    <div class="flex flex-col justify-center min-w-0 flex-grow">
                        ...
                        @if($author['bio'])
                            <p class="text-[11px] md:text-[13px] font-bold text-gray-300 leading-tight line-clamp-2 group-hover:text-gray-100 transition-colors max-w-xl">
                                {{ $author['bio'] }}
                            </p>
                        @endif
                    </div>
