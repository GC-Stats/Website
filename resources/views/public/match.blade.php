                ...
                <template x-if="activeMap === 0">
                    <div class="space-y-12 animate-fadeIn">
                        <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
                            @include('public.partials.team-stats-table', ['stats' => $totalA, 'teamName' => $teamAName, 'multiple' => true])
                            @include('public.partials.team-stats-table', ['stats' => $totalB, 'teamName' => $teamBName, 'multiple' => true, 'reverse' => true])
                        </div>
                ...
                @if($match['status'] != "upcoming")
                    @foreach($match['game_maps'] as $map)
                        <div x-show="activeMap === {{ $map['id'] }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-12">
                            ...
                            <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
                                @include('public.partials.team-stats-table', ['multiple' => false, 'stats' => $mStatsA, 'teamName' => $teamAName])
                                @include('public.partials.team-stats-table', ['multiple' => false, 'stats' => $mStatsB, 'teamName' => $teamBName, 'reverse' => true])
                            </div>
