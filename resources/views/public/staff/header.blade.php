{{--
    GC-Stats — Staff header partial

    Renders the staff member's profile header (photo, handle, country, bio,
    liquipedia, socials) and the tab nav — mirrors public.player's header
    block, minus the change-request banner. Only one tab exists today
    (Overview); kept as a nav for visual/structural consistency with
    player/team/organization and to make adding sub-pages later a drop-in.
...
<div class="mb-6">
    <div class="block group">
        <div class="relative bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 group-hover:bg-white/[0.04] group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)]">
            <div class="p-4 md:p-6 flex flex-col gap-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full">
                    <div class="flex items-center gap-4 md:gap-5 w-full">
                        <div class="relative flex-shrink-0">
                            @if ($staffMember->photo)
                                <img src="{{ $staffMember->photo }}" alt="{{ $staffMember->handle }}"
                            ...
                            @else
                                <div class="w-16 h-16 md:w-32 md:h-32 flex items-center justify-center border border-white/10 rounded-lg bg-[var(--brand-yellow)]/10">
                                    <span class="text-2xl md:text-4xl font-black text-[var(--brand-yellow)]">
                                        {{ strtoupper(substr($staffMember->handle, 0, 1)) }}
                                    </span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="flex flex-col justify-center min-w-0 flex-grow">
                            @if (trim("{$staffMember->first_name} {$staffMember->last_name}"))
                                <p class="text-gray-300 text-sm font-medium mb-1 group-hover:text-gray-100 transition-colors">
                                    {{ trim("{$staffMember->first_name} {$staffMember->last_name}") }}
                                </p>
                            @endif

                            <div class="flex items-center gap-3">
                                @if ($staffMember->country_code)
                                    <span class="fi fi-{{ blank($staffMember->country_code) || $staffMember->country_code === 'inter' ? 'un' : strtolower($staffMember->country_code) }} shadow-sm flex-shrink-0"
                                          aria-label="{{ $staffMember->country_code }}" role="img"></span>
                                @endif
                            ...
                            @if ($staffMember->player)
                                <a href="{{ route('players.show', [$staffMember->player->id, str($staffMember->player->handle)->slug()]) }}"
                                   class="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-gc-yellow hover:text-white transition mt-2 w-fit">
                                    {{ __('staff.also_plays_as', ['player' => $staffMember->player->handle]) }}
                                </a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap md:flex-col gap-2 w-full md:w-auto md:min-w-[180px] md:pl-6">
                        ...
                        @if(!empty($socials) || $staffMember->liquipedia_link)
                            <div class="flex flex-col gap-1 min-w-[150px] justify-center border-t md:border-t-0 md:border-l border-border-subtle pt-4 md:pt-0 md:pl-6">
                                @if($staffMember->liquipedia_link)
                                    <a href="{{ $staffMember->liquipedia_link }}" target="_blank" rel="noopener noreferrer"
                                       aria-label="Liquipedia: {{ $staffMember->handle }}"
            ...
            <nav aria-label="{{ __('staff.nav.aria_label') }}" class="bg-black/20 border-t border-white/5 overflow-x-auto no-scrollbar">
                <div class="flex justify-start md:justify-center min-w-max md:min-w-0">
                    @php
                        $navItems = [
                            ['route' => 'staff.show', 'label' => __('staff.nav.overview')],
                            ['route' => 'staff.experience', 'label' => __('staff.nav.experience')],
                        ];
                    @endphp

                    @foreach($navItems as $item)
                        @php $isActive = request()->routeIs($item['route']); @endphp
                        <a href="{{ route($item['route'], [$staffMember->id, str($staffMember->handle)->slug()]) }}"
