{{--
    GC-Stats — Link channels to matches wizard (shared content)

    Shared between admin/streams/matches/create.blade.php (extends
    admin.layout) and organization/dashboard/streams/matches/create.blade.php
    (extends organization.layout). The search endpoints are fixed admin.*
    routes regardless of context (already organization-agnostic — see
    Admin\MatchStreamController's docblock), but the final "link" submit
    uses $routePrefix so a successful dashboard submission redirects back
    into the dashboard instead of the admin list. Expects $backUrl,
    $routePrefix, $organization (nullable).
    ...
    without going through the admin match list, which organization members
    can't reach. Once a tournament is picked, every one of its matches is
    fetched once and rendered as a sortable table — same columns as
    ...
    <div x-show="tournament" x-cloak>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <form method="POST" action="{{ route($routePrefix.'link', $organization ?? []) }}" class="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4 sticky top-6">
                    @csrf
