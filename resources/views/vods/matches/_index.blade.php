{{--
    GC-Stats — Matches with a linked VOD, list all (shared content)

    Shared between admin/vods/matches/index.blade.php (site-admin view, flat
    across every organization, extends admin.layout) and
    organization/dashboard/vods/matches/index.blade.php (scoped to one
    organization, extends organization.layout) — same
    Admin\MatchVodController::index() action renders either wrapper
    depending on which route matched. The per-row edit/unlink forms always
    post to the fixed admin.matches.vods.* routes regardless of context —
    see that controller's docblock.

    Expects $matches, $sort, $direction, $countries, $vodsRestricted,
    $vodOrganizations, $organization (nullable — bound only in dashboard
    mode), and a $createUrl for the "add a VOD" button.
                                ...
                                <div class="flex items-center justify-between gap-3 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5">
                                    ...
                                    <x-modal :title="__('admin.vods.edit.title')">
                                        ...
                                        <form method="POST" action="{{ route('admin.matches.vods.update', [$match->tournament, $match, $vod]) }}" class="space-y-4">
                                            ...
                                            @if (! $vodsRestricted)
                                                <div>
                                                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5">{{ __('admin.vods.fields.organization') }}</label>
                                                    <x-styled-select name="organization_id" :selected="$vod->organization_id"
                                                        :options="collect(['' => __('admin.vods.fields.organization_none')])->union($vodOrganizations->mapWithKeys(fn ($organizationOption) => [$organizationOption->id => $organizationOption->name]))" />
                                                </div>
                                            @else
                                                <input type="hidden" name="organization_id" value="{{ $vod->organization_id ?? $vodOrganizations->first()?->id }}">
                                            @endif
